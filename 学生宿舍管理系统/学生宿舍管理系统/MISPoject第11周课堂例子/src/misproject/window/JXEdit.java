 ...
 * @author wjl
 */
public class JXEdit extends JDialog {
    ...
    public JXEdit(Window owner) {
        super(owner);
        initComponents();
        initTable();
    }
    /**
     * 为table对象中选中一行添加事件，在table中显示tb_User表所有记录。
     * 调用UserDao.allUsers()查询tb_User表所有记录
    ...
    /**
     * 以姓名和性别为查询条件查询user表
     * 调用UserDao.queryUsersByName()按姓名查询tb_User表记录
     * 然后调用getTableData()将结果转换为二维数组tableData。
     * 需要按功能调用对应dao包中类的对应方法。
     */
    private void userQuery(){
        ...
        list=null;
        if(sex.equals("全部")){
            list=UserDao.queryUsersByName(name);
        }else{
            list=UserDao.queryUsersByNameAndSex(name,sex);
        }
        showTableData(list);
        //修改、删除按钮不可用
        button4.setEnabled(false);
    ...
    /**
     * 将集合list转为二维数组,在table中显示，其中参数User表示list中元素为User类对象，
     * 需要按表对应类调整
     */
    private void showTableData(List<User> users){
        //表头
        String[]heads={"姓名","性别","简介","入职日期"};
        //表内容
        Object[][] tableData=new Object[users.size()][heads.length];
        int i=0;
        for(User user:users){
            tableData[i][0]=user.getName();
            tableData[i][1]=user.getSex();
            tableData[i][2]=user.getjianjie();
            tableData[i][3]=user.getruzhiriqi();
            i++;
        }
        //将表头和表内容显示到table1对象
    ...
    private User getUserfromUI() {
        ...
        if(textField4.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(null, "请输入入职日期");
            return null;
        }
        ...
        user.setjianjie(textField3.getText());
        user.setruzhiriqi(textField4.getText());
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (!e.getValueIsAdjusting()) {
            ...
            textField3.setText(list.get(currentIndex).getjianjie());
    ...
    private void buttonAdd(ActionEvent e) {
        ...
        if(n==0) {
            ...
            if(user==null )
                return ;
            int ok = UserDao.insertuser(user);
            if (ok == 0) {
    ...
    private void buttonModify(ActionEvent e) {
        ...
        if(n==0) {
            ...
            if(user==null )
                return ;
            user.setId(list.get(currentIndex).getId());
            int ok=UserDao.updateUser(user);
            if(ok==0) {
    ...
    private void buttonDel(ActionEvent e) {
        ...
        System.out.println(n);
        if(n==0) {
            int ok=UserDao.deleteUserById(list.get(currentIndex).getId());
            if(ok==0) {
